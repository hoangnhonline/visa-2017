...
<div class="box box-primary">
	<!-- form start -->
	<form id="frmMediaVideo" name="frmMediaVideo" role="form" action="{{ route('backend.media.video.store') }}" method="post">
		<div class="box-body">
			<div class="row">
    			<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
        			<div class="form-group">
                        ...
                        <div class="input-group input-group-sm">
                            <input type="text" class="form-control" id="media_filename" name="media_filename" value="{{ old('media_filename') }}" data-link="{{ route('backend.utils.videoinfo') }}" placeholder="Link video">
        			...
        			<div class="form-group">
        				...
        				<select class="form-control" id="media_source" name="media_source">
        					@foreach (config('cms.backend.media.source') as $name => $link)
                                <option value="{{ $name }}" data-link="{{ $link }}"{!! $name == old('media_source') ? ' selected="selected"' : '' !!}>{{ ucfirst($name) }}</option>
                            @endforeach
        				</select>
        			</div>
        			<div class="form-group">
        				<label for="menu_classname">Nhãn video</label>
        				<select class="form-control" data-width="100%" data-multiselect="true" data-ajax="1" data-url="{{ route('backend.utils.search.medialabel', ['t' => $type]) }}" data-placeholder="Chọn nhãn" data-tags="true" data-fields="label_name|label_name" id="media_label" name="media_label[]" multiple="multiple">
        					@foreach (old('media_label', []) as $label)
    			...
    			<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12 form-group">
					...
					<div class="embed-responsive embed-responsive-16by9" id="previewVideo">
						@if (old('media_filename'))
							<iframe class="embed-responsive-item" src="{{ old('media_filename') }}"></iframe>
						@endif
					</div>
    			</div>
            </div>
